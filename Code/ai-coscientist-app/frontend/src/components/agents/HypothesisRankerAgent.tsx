...
export function HypothesisRankerAgent({ output }: HypothesisRankerAgentProps) {
  ...
  const rankedHypotheses = pinnedText
    ? allRanked.filter((hyp: any) => isTracked(hyp.text || "") || isTracked(hyp.hypothesis || "") || isTrackedHypothesis(hyp, pinnedText))
    : allRanked;
  ...
  return (
    <Card className="p-4">
      <div className="flex items-start gap-3">
        <BarChart3 className="w-5 h-5 shrink-0 mt-0.5" style={{ color: getAgentColor("HypothesisRanker") }} />
        <div className="flex-1 space-y-3">
          <div className="flex items-start justify-between gap-2">
            <h4 className="font-semibold">
              Ranked {allRanked.length} Hypotheses{phaseLabel}
              {pinnedText && (
                <span className="ml-2 text-xs font-normal text-th-muted-fg">(showing 1 of {allRanked.length})</span>
              )}
            </h4>
          ...
          <div className="space-y-3">
            {(pinnedText ? rankedHypotheses : rankedHypotheses.slice(0, 5)).map((hyp: any, index: number) => {
