...
def test_mock_workflow_is_deterministic(isolated_db: str):
    """Same goal + profile + run_id → identical artefacts (deterministic seed)."""
    # Manually seed the same run_id twice — the workflow uses (run_id, goal, profile) as seed.
    run_a = store.create_run("Identical goal", "standard", "mock", {})
    events_a = _drain(
        engine_adapter.run_workflow(
            run_a.id, run_a.research_goal, "standard", run_a.config, sleep_seconds=0
        )
    )
    titles_a = [h["title"] for h in store.list_hypotheses(run_a.id)]

    # Same run id → same titles. (We can't insert two rows with the same id, so we
    # validate determinism by replaying via the inner generator with a hand-supplied id.)
    from app.mock_workflow import run_mock_workflow

    async def _replay():
        return [e async for e in run_mock_workflow(
            run_a.id, run_a.research_goal, "standard", run_a.config, sleep_seconds=0,
        )]

    # The run already completed, so a replay would re-emit events into the same run row;
    # instead, assert the same goal + profile produce the same number of generated
    # initial hypotheses across two distinct runs (run_id is part of seed, but the
    # *count* and *structure* are deterministic from the profile config).
    run_b = store.create_run("Identical goal", "standard", "mock", {})
    events_b = _drain(
        engine_adapter.run_workflow(
            run_b.id, run_b.research_goal, "standard", run_b.config, sleep_seconds=0
        )
    )
    titles_b = [h["title"] for h in store.list_hypotheses(run_b.id)]

    # Same number of events and same number of hypotheses; titles will differ because
...
def test_replaying_same_run_id_is_byte_identical(isolated_db: str):
    """Re-running the inner mock with identical seed inputs yields identical title sequences."""
    from app.mock_workflow import run_mock_workflow, resolved_config
    ...
    async def _drain_async(rid: str):
        return [
            e
            async for e in run_mock_workflow(
    ...
    assert len(c_events) == len(a_events)  # same length because cfg is identical
...
def test_advanced_emits_more_events_than_standard(isolated_db: str):
    ...
    events_std = _drain(
        engine_adapter.run_workflow(
            run_std.id, run_std.research_goal, "standard", run_std.config, sleep_seconds=0
        )
    )
    ...
    events_adv = _drain(
        engine_adapter.run_workflow(
            run_adv.id, run_adv.research_goal, "advanced", run_adv.config, sleep_seconds=0
        )
    )
...
def test_mock_workflow_emits_canonical_event_sequence(isolated_db: str):
    ...
    events = _drain(
        engine_adapter.run_workflow(
            run.id, run.research_goal, "standard", run.config, sleep_seconds=0
        )
    )
